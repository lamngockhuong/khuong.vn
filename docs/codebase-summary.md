...
│   └── apps/                      # Mini-apps framework
│       ├── index.html             # Apps listing page
│       ├── main.ts                # Apps listing entry & grid renderer
│       ├── styles.css             # Apps listing page styles
│       ├── apps.json              # App registry configuration
│       └── lucky-wheel/           # Lucky Wheel mini-app
│           ├── index.html         # Wheel app HTML
│           ├── main.ts            # Event handlers & initialization
│           ├── wheel.ts           # LuckyWheel class (Canvas implementation)
│           └── styles.css         # Wheel-specific styles
│
├── functions/                     # Cloudflare Pages Functions
│   ├── _middleware.ts             # Request routing middleware
│   └── redirects.ts               # Short link redirect mappings
│
├── public/                        # Static assets (favicons, images, sitemap, robots.txt)
├── scripts/                       # Build scripts
│   └── generate-sitemap.ts        # Auto-generate sitemap.xml from apps.json
├── dist/                          # Build output (generated by Vite)
│
├── config.json                    # Site configuration (name, role, links, SEO)
    ...
    "apps": [
      {
        ...
        "description": "Quay số ngẫu nhiên...",
        "icon": "🎡",
...
- **Build Targets**: 3 entry points → 3 HTML files + shared assets
...
apps/index.html
  ↓
apps/main.ts ← apps.json
  ↓
apps/styles.css

apps/lucky-wheel/index.html
  ↓
apps/lucky-wheel/main.ts ← wheel.ts
  ↓
wheel.ts (Canvas rendering & animation)
  ↓
apps/lucky-wheel/styles.css

functions/_middleware.ts ← redirects.ts
  ↓
HTTP redirects
...
└── assets/
    ...
    ├── theme-{hash}.css           # Shared theme styles
    └── modulepreload-polyfill-{hash}.js
```

## Extension Points

1. **Add New Theme**: Edit `src/themes.ts` → add renderer function, register in `config.themes.enabled`
2. **Add Short Link**: Edit `functions/redirects.ts` → add entry
3. **Add Mini-App**: Create `src/apps/{app-name}/` → register in `src/apps/apps.json` → add entry in `vite.config.ts` rollupOptions
4. **Customize Site Content**: Edit `config.json` → rebuild (name, role, links, SEO, analytics)
